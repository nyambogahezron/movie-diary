...
class EmailService {
    ...
    static async sendNewLoginAlert(user, ipAddress, deviceInfo) {
        const mailOptions = {
            from: this.fromEmail,
            to: user.email,
    ...
    static async sendEmailChangeNotification(user, oldEmail, newEmail) {
        const mailOptions = {
            from: this.fromEmail,
            to: oldEmail,
...
EmailService.transporter = nodemailer_1.default.createTransport((0, NodeMailer_1.default)());
EmailService.fromEmail = process.env.EMAIL;
EmailService.clientUrl = process.env.CLIENT_URL;
